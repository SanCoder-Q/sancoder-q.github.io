...
- 线性递归计算 Linear Recursion Process (*NOT Procedure!!*)
#+BEGIN_SRC scheme
  (define (factorial n)
    (if (= n 1)
        1
...
- 线性迭代计算 Linear Iteration Process (*NOT Procedure!!*)
#+BEGIN_SRC scheme
...
计算 (Process) 描述的是计算的进展方式。一个递归的过程很可能是以一种迭代进展方式计算的，也就是 *尾递归 (Tail-Recursion)* 的方式计算的。
